...
- **Bug corrigé — `.lueurs/config.json` écrasé au cold start** : `ensureVaultConfig()` (`vaultConfig.ts`) est appelé à chaque démarrage. Avant le fix, tout échec de lecture du marqueur (`readVaultConfig` catch-all → `null`) était traité comme "fichier absent" et déclenchait la réécriture d'une config vide (`spaces: []`, nouveau `vaultId`) — y compris quand le fichier existait mais n'était pas encore matérialisé par iCloud (placeholder pas téléchargé juste après un cold start iOS). Cette écrasure se resynchronisait ensuite vers tous les devices, faisant "disparaître" les espaces sans action utilisateur. Fix : `ensureVaultConfig` vérifie maintenant `exists()` avant de recréer — si le fichier existe mais reste illisible, une seule nouvelle tentative après 1s, sinon abandon **sans jamais écraser**.
...
- `renameEnumValue { key, old_value, new_value }` — renomme une valeur d'un bouton : met `key` à `new_value` chez les héritiers qui valaient exactement `old_value` (voir [Propriétés à valeurs contraintes (ENUM)](#propriétés-à-valeurs-contraintes-enum))
...
**Champ d'édition partagé (`FormulaEditField`).** La logique de saisie (input humanisé + détection des triggers `ref(`/`ref("…").`/`self.` + `NoteSelector`/`PropertySelector` + `selectorOpenRef` anti-stale-closure) a été extraite de `FrontmatterValue` vers `src/shared/components/FormulaField/FormulaEditField.tsx`, partagé entre le frontmatter et le popup inline. `FrontmatterValue` n'a plus que l'auto-pair `$$` et la vue compactée ; il délègue l'édition à ce composant.

Deux points d'adaptation au contexte appelant, le reste étant commun :
- `refPathOf(note)` — chemin écrit dans `ref()` : `note.id` (absolu) pour le frontmatter, relatif au vault pour le corps. Il alimente la table `notesByName` utilisée par `dehumanizeFormula`.
- `selfProperties` — options du `PropertySelector` pour `self.` : clés de `formulaVars` **moins la propriété en cours d'édition** côté frontmatter (une propriété ne se référence pas elle-même), clés de `inlineFormulaBridge.vars` côté corps. Le trigger `SELF_PROP_TRIGGER_RE` borne à gauche sur `[^\p{L}\p{N}_.]` pour ne pas se déclencher sur `monself.` ni sur `ref("X").self.`.

**Brouillon local pendant la frappe (les deux appelants).** `FormulaEditField` est un composant contrôlé : il appelle `onChange` à chaque caractère. Le popup inline absorbe ça dans un `useState` local et n'écrit dans le document (`setNodeAttribute`) qu'à la validation. Le frontmatter faisait l'inverse — `onTextChange` → `commit()` à chaque frappe, soit `setRows` + `onChange(toFrontmatter(rows))` : réécriture de la note, re-render de toutes les lignes, et un `computeFormula` par formule (donc un `agg()` sur tous les enfants si la note est une base). Latence de saisie très visible.

`FrontmatterValue` tient donc lui aussi un `formulaDraft` local et ne commit qu'à `onDone`. Deux détails qui vont avec :
- l'évaluation `computeFormula` de la valeur affichée est placée **après** la branche d'édition — inutile de recalculer un résultat masqué à chaque frappe ;
- un filet en `useEffect` de démontage rejoue le brouillon : les sorties normales (Entrée, Échap, clic ailleurs) passent par `onDone`, mais un démontage direct (changement de note au clavier, rechargement par le watcher) perdrait sinon la saisie.

Effet de bord bienvenu : les valeurs intermédiaires ne remontent plus dans l'atom, donc effacer une formule en cours d'édition ne fait plus basculer le champ en mode texte au milieu de la frappe.

**Échap valide, il n'annule pas — décision assumée.** Entrée, Échap, clic ailleurs et fermeture du popup mènent tous à `onDone`, donc au même commit. C'était mécaniquement le cas avant le brouillon (chaque frappe étant déjà commitée, il n'y avait rien à annuler) ; ça reste vrai par choix. Le brouillon local rendrait pourtant l'annulation triviale — il suffirait de ne pas appeler `onChange` sur la branche Échap. Si le comportement paraît incohérent en relisant le code, c'est une décision, pas un oubli : ne pas le « corriger » sans arbitrage.

**Piège — identité des props dans les mémos.** `refPathOf` passé en **paramètre par défaut** (`refPathOf = (note) => note.id`) recrée la fonction à chaque render et invalide le mémo `notesByName`, qui reconstruit alors une `Map` de toutes les notes du vault à chaque caractère tapé. Défaut extrait en constante module-level (`noteIdPath`).

**Piège — caret perdu au passage input → textarea.** Dans le frontmatter, l'auto-pair `$$` se fait dans l'`<input>` texte ; c'est le **premier caractère saisi** qui rend la valeur conforme à `isFormula` et fait basculer le rendu sur `FormulaEditField`, un `<textarea>` — un autre élément DOM, donc un caret neuf en position 0. Sans repositionnement, le caractère suivant s'insérait *avant* celui qu'on venait de taper (`|1` puis `+1`). `FormulaEditField` place donc systématiquement le caret en fin de valeur à son montage (effet `[]`) ; ça vaut aussi pour le popup inline, qui s'ouvre lui aussi sur un élément fraîchement monté.
...
**Round-trip (`remark-inline-formula.ts`).** Lecture : `$$expr$$` → nœud mdast `inline_formula` { value }. Écriture : handler remark-stringify renvoyant `node.value` **verbatim** — crucial pour ne pas échapper les `*`, `_`, `[` d'une expression (un nœud texte mdast les échapperait → casse du round-trip). Une formule vide (`$$$$`, édition en cours) est sérialisée en chaîne vide (jamais persistée comme littéral).

**Lecture depuis la source, pas depuis l'arbre (`source-spans.ts`).** Le contenu d'une formule n'a aucun sens markdown, mais remark l'ignore et parse son intérieur comme du texte : un `*`, `_`, `[` ou `~` dans un chemin `ref("…")` — c'est-à-dire dans n'importe quel nom de note un peu exotique — engendre des nœuds `emphasis`/`delete`/`link` **au milieu** de la formule. Elle n'est alors plus contenue dans un seul nœud `text` : un scan par nœud ne la voit plus, et elle s'affiche en texte brut, `$$` compris.

Les caractères fautifs ont disparu de l'arbre (remark les a consommés comme délimiteurs) — impossible de reconstituer la formule depuis le mdast seul. Mais les **offsets** survivent (`position.start/end.offset`), et Milkdown appelle `remark.runSync(tree, markdown)` : le transformer reçoit donc la source en 2e argument. `rebuildWithFormulas(children, source)` repère les `$$…$$` dans la tranche de source couverte par les enfants d'un nœud inline, puis remplace les nœuds concernés — ceux entièrement contenus dans une formule sont absorbés, les nœuds texte à cheval sur une frontière sont découpés par offset.

Garde-fous :
- une formule entièrement contenue dans un **seul nœud non-texte** (`*du $$1+1$$ en italique*`) est laissée à la récursion sur ce nœud, sinon on la traiterait deux fois ;
- un nœud texte à cheval n'est découpé que si `end - start === value.length`, c'est-à-dire en l'absence d'échappement (`\*`) ou d'entité (`&amp;`) qui décaleraient les offsets ;
- tout cas ambigu renvoie `null` → repli sur le scan par nœud texte (`scanTextNodes`), le comportement historique. Vérifié : sur source échappée ou avec entité HTML, le repli retrouve bien la formule.

Ce mécanisme rend `singleTilde: false` **redondant** pour les formules (il corrige le cas iCloud à lui seul, vérifié avec les options gfm par défaut). Les deux sont conservés : le premier évite en plus qu'un `~` isolé dans du texte ordinaire (`~10 ou ~15 personnes`) soit rendu barré à tort.

**Contexte d'évaluation (`inlineFormulaBridge`).** Ref module-level (pattern `wikilinkBridge`) alimentée par `MarkdownEditor` : `vars` = frontmatter de la note courante (`self.*`), `noteResolver`/`allNotes` depuis `notesByIdAtom`, `children` résolus depuis `__Children__` pour `agg()`, `vaultPath` pour relativiser les `ref()` à la sérialisation.

**Chemins des `ref()` du corps : relatifs, pas absolus (`refPaths.ts`).** Les formules inline suivent la convention du **corps de note** (images, audio, liens entre notes) : chemin relatif à la racine du vault, sur disque **comme dans l'attribut `raw` du nœud**. C'est l'inverse du frontmatter, qui garde ses `ref()` absolus en mémoire et ne les relativise qu'à l'écriture (`relativizePathFields`).

Ce n'est pas une question de goût : Milkdown parse le body **en mémoire**. Avec la convention du frontmatter, un chemin absolu de conteneur iCloud (`…/Mobile Documents/iCloud~com~lueurs~app/Documents/…`) serait présent au moment du parse — or `remark-gfm` interprète le **tilde simple** comme du barré (`singleTilde: true` par défaut). Le nœud texte est alors découpé en `text` + `delete` + `text`, et le scan `$$…$$` de `remark-inline-formula.ts` (qui n'examine que les nœuds `text` entiers) ne matche plus : la formule s'affiche en texte brut, `$$` compris, avec un morceau de chemin barré.

Trois verrous, indépendants :
- lecture des formules depuis la source plutôt que depuis l'arbre (`source-spans.ts`, § dédié plus bas) — c'est celui qui couvre *tous* les caractères problématiques, pas seulement le tilde ;
- `ctx.set(remarkGFMPlugin.options.key, { singleTilde: false })` dans `MarkdownEditor` — seul `~~texte~~` est du barré. L'*input rule* de Milkdown accepte toujours `~texte~` à la frappe (elle est côté ProseMirror, pas remark) et sérialise en `~~texte~~` : le round-trip reste cohérent. Effet de bord assumé : un `~texte~` littéral déjà sur disque n'est plus rendu barré.
- `relativizeRefPaths` appliqué dans le `toMarkdown` du nœud (`schema.ts`) — le disque ne reçoit que du relatif. `noteResolver` (`MarkdownEditor`) tente `vaultPath + chemin` puis retombe sur le chemin nu : l'absolu reste résolvable, ce qui couvre à la fois les notes écrites avant ce changement et les chemins `__Children__` (absolus en mémoire) que `ref()` traverse pour évaluer un `agg()` sur une base référencée.

Combinés, ils migrent automatiquement l'existant : les notes cassées reparsent correctement, puis leur première sérialisation réécrit les chemins en relatif.

**Évaluation et CSP.** `computeFormula` compile l'expression avec `new Function` — c'est-à-dire de l'`eval`. La CSP de `tauri.conf.json` doit donc porter `script-src … 'unsafe-eval'`, sans quoi l'appel lève une `EvalError` attrapée par le `catch` et **toutes** les formules affichent `#ERREUR`. Piège de diagnostic : le symptôme n'apparaît qu'en **build**. En `tauri dev` le front est servi par Vite (`http://localhost:1420`), qui ne passe pas par le protocole Tauri et n'applique donc aucune CSP.

**Saisie & popup.** Input rule `/\$\$$/` (`input-rule.ts`) → insère un nœud vide + pose `pendingOpenPos` ; un second plugin (`view.update`) ouvre alors le popup sur le nœud fraîchement inséré (le handler d'input rule n'a pas accès aux coordonnées écran). `InlineFormulaPopup` (singleton, monté dans `NoteEditor` à côté de `WikilinkEditPopup`) réutilise `FormulaEditField` ; à la validation, `setNodeAttribute(pos, "raw", …)` ou suppression si vide. Humanisation/déshumanisation par les mêmes fonctions que le frontmatter (`formulas.ts`), mais stockage en chemins **relatifs** (cf. § « Chemins des `ref()` du corps »).
...
`computeTemplateProps` (`fileTreeHelpers.ts`) calcule les propriétés qu'une note doit recevoir d'après ses templates. Elle distingue propriétés imposées (valeur non vide dans le template) et contraignantes (valeur vide), et ne modifie jamais une valeur déjà renseignée par la note pour une propriété contraignante. Pour une propriété ENUM, elle attribue le `default` à la création et réécrit toute valeur non permise vers le `default` (filet de sécurité au chargement, en complément de `enforceEnum` côté live).

### Propriétés à valeurs contraintes (ENUM)

> Renommé depuis BUTTON (`buttonProperty.ts` → `shared/lib/FrontmatterPicker/enumProperty.ts`, `ButtonOption`/`ButtonDef` → `EnumOption`/`EnumDef`) : le nom prêtait à confusion avec un composant bouton. Toute référence à BUTTON dans du code ou de la doc plus ancienne est obsolète.

`$$ENUM([v1;v2;v3],default)$$` définit une liste de valeurs autorisées. C'est une contrainte **déclarative** : contrairement aux formules `round`/`agg`/`ref`, un ENUM n'est jamais évalué dans le sandbox. La note ne stocke que la valeur littérale choisie (jamais la formule) — sauf si l'ENUM est défini directement sur la note elle-même (pas de template), auquel cas c'est la formule `$$ENUM(...)$$` qui EST la valeur stockée, avec `default` qui porte la sélection courante (cf. « ENUM sans template » plus bas).

**Parsing (`enumProperty.ts`).** `parseEnum` renvoie `{ options: EnumOption[]; default }`, où `EnumOption = { value; color? }`. Séparateur `;` à l'intérieur des crochets (évite le conflit avec la `,` des arguments). `default` vide → première valeur. Une option peut être colorée via `=={color}label==` / `==label==` (syntaxe reprise du surlignage) ; couleur omise → `defaultHighlightColorRef.current` (réglage courant). `diffEnumOptions` compare **par valeur** (la couleur n'affecte pas les héritiers) avec la même heuristique 1-retirée/1-ajoutée que `diffFrontmatter` pour détecter un renommage.

**Contraintes issues d'un template.** `useTemplateConstraints` expose, en plus de `lockedKeys`/`lockedValues`, une `Map<key, EnumDef>` (`enumConstraints`) : une clé ENUM est verrouillée mais **pas** dans `lockedValues` (valeur éditable). `useTable` calcule de même un `enumConstraint` par colonne via `computeTemplateConstraints` (`shared/lib/FrontmatterPicker/templateConstraints.ts`, pure, testée, partagée avec le panneau frontmatter).

**Propagation des changements du template.** `diffFrontmatter` (`useTemplateSync`) détecte qu'une valeur reste un ENUM entre `prev` et `next` (n'émet donc jamais de `forceValue`) et, via `diffEnumOptions`, pousse un `renameEnumValue` par renommage de valeur et un `enforceEnum` si des options ont disparu. Un `addProp` d'un ENUM envoie le `default` (pas la formule). La mise à jour chirurgicale du tree applique la même logique localement.

**Rendu.** `EnumValueSelector` (`shared/components/FrontmatterPicker`) — pill + dropdown (`EnumOptionsDropdown`, réutilise `AnchoredDropdown`, donc compatible mobile) — affiche la valeur dans sa couleur (mapping highlight→Tailwind dans `enumPillColors.ts`, fond `-400`/texte `-600`, `purple`→`purple`, `yellow`→`amber`). Utilisé dans `FrontmatterValue`, `TableCell` et `MobileTableCell`. `enumValueState` distingue `valid` / `placeholder` (valeur = default hors-liste) / `invalid` (signalé en rouge, en attendant la réécriture).

**Important — `value` de `EnumValueSelector` n'est jamais la formule brute.** Le composant attend le **libellé sélectionné** (`def.default`), jamais `$$ENUM(...)$$` tel quel — comparer une formule brute aux `options` échoue systématiquement (`enumValueState` renvoie `invalid`). `onChange` doit resérialiser toute la définition (`serializeEnum({ ...def, default: v })`), pas écrire le libellé choisi comme valeur brute (ce qui détruirait la formule ENUM, en retombant sur une propriété texte). Bug réel rencontré en ajoutant le pill aux cellules de tableau (cf. « ENUM sans template » plus bas) — à surveiller sur tout futur appelant.

#### Réglages Texte/Nombre/Bouton (roue crantée)

Le panneau qui permet de choisir/éditer un ENUM sans taper `$$ENUM(...)$$` à la main est un switcher à 3 modes (Texte/Nombre/Bouton), présent à deux endroits distincts avec deux mécaniques d'état différentes :

- **Frontmatter (`FrontmatterRow`/`useValueEditor.ts`)** — panneau qui s'ouvre **inline**, au-dessus/en dessous de la ligne icônes+champ (pour que le champ ne bouge jamais visuellement, cf. `useAnimatedHeight`). État tenu dans un atome global `settingsKeyAtom` keyé par nom de champ (une seule propriété éditée à la fois dans tout le panneau frontmatter). Roue crantée (`canConfigure = !locked && !row.isSystem && !isValueLocked && !enumConstraint`) — masquée pour un héritier ENUM contraint par un template (la valeur se choisit directement sur la ligne via `EnumValueSelector`, jamais ce panneau).
- **Cellule de tableau (`usePropertyCellSettings`/`PropertyCellSettingsPopup`, `shared/components/FrontmatterPicker/`)** — panneau en **popup ancré** (`AnchoredDropdown`, même mécanique que `NumberCellSelector`), état **local à la cellule** plutôt qu'un atome global keyé par nom de champ : une même clé de colonne existe sur plusieurs notes/lignes, un atome global confondrait leurs éditions.

Les deux réutilisent la même logique de brouillon pure, extraite dans `shared/lib/FrontmatterPicker/propertyDraft.ts` (testée) : `makeInitialDraft` (déduit Texte/Nombre/Bouton depuis la valeur committée), `changeDraftType` (bascule de type en conservant le contenu tapé, sauf Bouton qui repart d'une liste vide), `withFormatConstraint` (impose decimals/unit d'un template), `serializeDraft` (sérialisation finale). `useValueEditor.ts` délègue à ce module au lieu de dupliquer cette logique ; `usePropertyCellSettings` fait de même sans avoir besoin de gérer de contrainte ENUM de template (la roue crantée d'une cellule est masquée dès qu'un `enumConstraint` de template existe sur la colonne).

Le contenu du panneau (SegmentedControl + champ Nombre/`NumberFormatFields` ou `EnumOptionsFields`) est lui aussi partagé : `PropertyModeFields` (`shared/components/FrontmatterPicker/`), extrait du corps de `InlineFormulaEditPopup` (popup de formule inline du corps de note, qui n'expose que Nombre/Bouton — un nœud de formule inline n'est jamais « juste du texte ») et généralisé avec un mode Texte optionnel pour le panneau de cellule.

**Taper `$$` dans un champ/cellule Texte libre** ouvre ce panneau (préréglé Nombre, expression vide) au lieu de basculer directement en édition de formule brute — `openFormulaEditor()` côté frontmatter, `openAsFormula()` côté cellule (même brouillon vierge : `makeEmptyFormulaDraft`).

#### ENUM sans template

Avant cette extension, `TableCell`/`MobileTableCell` ne détectaient un ENUM que via `enumConstraint` (calculé depuis les templates de la base) : un ENUM défini directement sur une note enfant, sans template, retombait sur l'affichage générique `ƒ <formule brute>`. Fix : reparser la valeur brute de la cellule avec `parseEnum` en repli quand `enumConstraint` est absent (même logique que `FrontmatterValue`, qui le faisait déjà pour le frontmatter — cf. sa branche « Propriété ENUM non contrainte »).

**Course pill ↔ panneau de réglages.** Une fois le pill affiché pour un ENUM sans template, la roue crantée à côté permet d'éditer ses options via le même panneau que ci-dessus. Piège : si le pill (cliquable indépendamment, à gauche du popup) committe directement via `onCommit` pendant que le panneau de réglages est ouvert à côté (brouillon local, pas encore committé), fermer le panneau resérialise ce brouillon désormais périmé et écrase le choix fait entre-temps sur le pill. Fix : quand le panneau est ouvert en mode Bouton, le pill lit/écrit le brouillon (`cellSettings.draft.enumDef`) au lieu de committer directement — même principe que `FrontmatterValue.isEditingEnum`, qui liait déjà le pill au brouillon en cours plutôt qu'à la valeur committée.
...
### Caret et saisie aux bords des marques « à boîte »

Trois marques inline ont un **décalage horizontal visuel** entre intérieur et extérieur (« boîte ») : `inlineCode` (padding + fond + police `0.875em`), `didascalie_inline` et `highlight_inline` (pseudo-pipes `| |` / padding). À leurs frontières, une même position de document correspond à **deux emplacements visuels** (dedans / dehors) ; selon l'inclusivité de la marque, certains de ces slots de saisie sont par défaut inatteignables. Quatre mécanismes coordonnés règlent ça.

**Rendu — `caretSide` (`customCaretPlugin`).** Le caret est un `<div>` positionné via `coordsAtPos`, pas le caret natif (`caret-color: transparent`). `coordsAtPos(pos, side)` renvoie des coordonnées différentes de part et d'autre d'une frontière de boîte. `caretSide(state)` choisit le `side` selon **où le glyphe va s'insérer** : le prochain caractère portera `storedMarks ?? $from.marks()` ; s'il partage la marque-frontière avec le contenu de gauche → `-1` (collé à gauche), sinon `+1`. Sans effet sur gras/italique (pas de boîte → `-1` et `+1` coïncident). Le même `side` est passé à `domAtPos` pour que la hauteur du caret reflète la bonne `font-size` (police réduite dans le code).

**Atteignabilité — `crossBoxEdge` + `escapeInlineMarksPlugin`.** `inlineCode` est **inclusive** (piège « dedans » au bord droit, « dehors » au bord gauche) ; didascalie/highlight sont **non-inclusives** (cachent les deux slots « dedans », avant le 1er et après le dernier caractère). `crossBoxEdge(state, neighbor)` détecte une frontière de boîte (appartenance d'une `BOX_MARK_NAMES` différente entre l'effectif et le nœud voisin) et renvoie les stored marks ajustées en **adoptant l'appartenance du voisin** (entrer avec ses attrs via `mark.addToSet`, sortir via `mark.removeFromSet`). `escapeInlineMarksPlugin` l'appelle sur `ArrowRight` (voisin = `nodeAfter`) et `ArrowLeft` (voisin = `nodeBefore`) : 1re flèche bascule les stored marks (caret immobile, slot caché révélé), 2e flèche navigue. `ArrowRight` conserve l'échappement de **fin de bloc** pour les autres marques inclusives du preset (gras/italique/barré) : `setStoredMarks([])`. Le slot caché étant toujours atteint en allant vers la droite côté gauche d'une boîte, l'« entrée » se fait sur `ArrowRight` (le caret y va physiquement vers la droite).

**Stickiness — `stickyInlineBoxPlugin`.** Les marques non-inclusives sont abandonnées après chaque caractère tapé en **bout** de boîte (caret éjecté dehors → il faudrait re-rentrer à chaque frappe). Un `appendTransaction` remet la marque dans les stored marks après une frappe qui (a) porte la marque sur le caractère précédent mais pas le suivant (= extension par la droite) et (b) **était déjà active avant la frappe** (`oldState`). La condition (b) est cruciale : sans elle, l'input-rule qui *crée* `||x||` en fin de ligne piégerait l'utilisateur dedans juste après création. `inlineCode` (inclusive) est collante nativement → ignorée via `spec.inclusive !== false`.

**Correction orthographique — préservation des marques.** Le remplacement d'un mot fautif (`useContextMenu.ts` desktop, `MobileSpellMenu.tsx` mobile) utilisait `schema.text(replacement)` (sans marques) → scindait la mise en forme quand le mot était dans une didascalie/surlignage/code (`||testons une chose||` corrigé devenait `||testons|| une ||chose||`). Fix : `doc.resolve(from).marksAcross(doc.resolve(to))` récupère les marques couvrant tout le mot, passées à `schema.text(replacement, marks)`.
