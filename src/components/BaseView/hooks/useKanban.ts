...
export function getAvailableKanbanKeys(base: NoteFile, allNotes: NoteFile[]): string[] {
    const templatePaths = base.frontmatter[SystemField.TEMPLATE];
    if (!templatePaths || !Array.isArray(templatePaths) || templatePaths.length === 0) return [];
    ...
    return templatePaths.flatMap((path) => {
        ...
        if (!template) return [];
        return getFreeProps(template.frontmatter).filter((k) => !seen.has(k) && seen.add(k));
    });
}

/** Valeurs possibles d'une propriété libre à travers les templates de la base. */
export function getTemplateValues(base: NoteFile, allNotes: NoteFile[], key: string): string[] {
    const templatePaths = base.frontmatter[SystemField.TEMPLATE];
...
export function useKanban({ base, onBaseChange }: UseKanbanProps) {
    ...
    const childNotes = (() => {
        const paths = Array.isArray(base.frontmatter[SystemField.CHILDREN])
            ...
            : [];
        return paths.map((p) => allNotes.find((n) => n.id === p)).filter((n): n is NoteFile => !!n);
    })();
    ...
    const [optimisticCards, setOptimisticCards] = useState<KanbanCards | null>(null);
    ...
    const initKanban = useCallback((key: string) => {
        ...
        const distinctValues = fromNotes.length > 0
            ? fromNotes
    ...
    const moveCard = useCallback(async (noteId: string, fromColId: string, toColId: string) => {
        ...
        if (!toCol) { log.warn("colonne cible introuvable", { toColId }); return; }

        const note = childNotes.find((n) => n.id === noteId);
        if (!note) { log.warn("note introuvable pour déplacement", { noteId }); return; }

        log.info("déplacement carte", { noteId, fromColId, toColId, newLabel: toCol.label });
        ...
        setOptimisticCards((prev) => {
            ...
            return {
                ...
                [toColId]: [
                    ...(base[toColId] ?? []),
                    { ...note, frontmatter: { ...note.frontmatter, [kanbanKey]: toCol.label } },
                ],
            };
        });
    ...
    const addColumn = useCallback((label: string) => {
        ...
        onBaseChange({
            ...base.frontmatter,
            [SystemField.KANBAN_COLUMNS]: serializeColumns([...persistedColumns, newCol]),
        });

        setOptimisticCards((prev) => ({ ...(prev ?? derivedCardsRef.current), [newCol.id]: [] }));
    }, [base, persistedColumns, onBaseChange]);

    const renameColumn = useCallback(async (colId: string, newLabel: string) => {
        ...
        onBaseChange({
            ...
            [SystemField.KANBAN_COLUMNS]: serializeColumns(
                persistedColumns.map((c) => c.id === colId ? { ...c, label: newLabel } : c)
            ),
        });

        const notesToPatch = childNotes.filter((n) => n.frontmatter[kanbanKey] === col.label);
        log.info("patch notes suite au renommage", { count: notesToPatch.length, oldLabel: col.label });
    ...
    const reorderColumns = useCallback((newOrder: KanbanColumn[]) => {
        ...
        onBaseChange({ ...base.frontmatter, [SystemField.KANBAN_COLUMNS]: serializeColumns(newOrder) });
    }, [base, onBaseChange]);
