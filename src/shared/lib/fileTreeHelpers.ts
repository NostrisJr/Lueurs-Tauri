...
export function ensureType(
  ...
  const inferredType =
    noteName === parentFolderName ? NoteType.FOLDER : NoteType.NOTE;
  ...
  while (i < lines.length) {
    ...
    if (value === "") {
      ...
      if (items.length > 0) {
        ...
        frontmatter[key] =
          joined.startsWith("$$") && joined.endsWith("$$") ? joined : items;
      } else {
        frontmatter[key] = "";
      }
      continue;
...
export function updateNodeInTree(
  ...
  return nodes.map((node) => {
    ...
    if (node.kind === "folder")
      return {
        ...node,
        children: updateNodeInTree(node.children, fileId, patch),
      };
    return node;
  });
}
...
export function addNodeInTree(
  ...
  return nodes.map((node) => {
    ...
    if (node.kind === "folder") {
      return {
        ...node,
        children: addNodeInTree(node.children, parentId, newNode, rootId),
      };
    }
    return node;
  });
