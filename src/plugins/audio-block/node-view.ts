...
export function createAudioBlockNodeView(config: AudioBlockConfig = {}) {
  return function audioBlockNodeView(
    ...
    titleEl.textContent =
      currentNode.attrs.title || currentNode.attrs.src || "Audio";
    ...
    audio.addEventListener("error", () => {
      log.error("erreur lecture audio", {
        src: audio.src,
    ...
    return {
      ...
      update(updatedNode: ProsemirrorNode) {
        ...
        if (updatedNode.attrs.src !== currentNode.attrs.src) {
          log.info("src audio changé", {
            old: currentNode.attrs.src,
            new: updatedNode.attrs.src,
          });
          resolveAndSetSrc(updatedNode.attrs.src);
          srcHint.textContent =
            updatedNode.attrs.src.split(/[/\\]/).pop() ?? "";
        }
        ...
        if (
          ...
          titleEl.textContent =
            updatedNode.attrs.title || updatedNode.attrs.src || "Audio";
        }
      ...
      stopEvent(event: Event) {
        ...
        if (target === audio || audio.contains(target as globalThis.Node))
          return true;
        return false;
      },
