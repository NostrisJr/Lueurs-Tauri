...
import { reconcileNumberFormat } from "../../shared/lib/FrontmatterPicker/numberProperty";
import {
      ...
      setTree((prev) => {
        ...
        for (const path of affectedPaths) {
          ...
          if (change.type === "addProp" && !(change.key in fm)) {
            ...
            newFm = { ...fm, [change.key]: change.value };
          } else if (change.type === "forceNumberFormat") {
            const current = fm[change.key];
            const curStr = typeof current === "string" ? current : "";
            const reconciled = reconcileNumberFormat(curStr, change);
            if (reconciled !== curStr) {
              newFm = { ...fm, [change.key]: reconciled };
            }
